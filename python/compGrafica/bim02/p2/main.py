...
def desenhar_corpo_central_unico(raio_x, raio_y, profundidade, n_lados):
    """Desenha o corpo principal da nave mais achatado."""
    glColor3f(*COR_CORPO)
    ...
    for i in range(n_lados + 1):
        ...
        glVertex3f(x, y, profundidade / 4)  # Reduzindo profundidade para achatar
    glEnd()
    ...
    for j in range(6):
        ...
        glBegin(GL_QUADS)
        glVertex3f(raio_x * math.cos(angle1), raio_y * math.sin(angle1), profundidade / 4)
        glVertex3f(raio_x * math.cos(angle2), raio_y * math.sin(angle2), profundidade / 4)
        glVertex3f(raio_x * math.cos(angle2), raio_y * math.sin(angle2), -profundidade / 4)
        glVertex3f(raio_x * math.cos(angle1), raio_y * math.sin(angle1), -profundidade / 4)
        glEnd()
...
def desenhar_cauda():
    ...
    glVertex3f(-0.03, -0.02 + altura_conexao, -1.0)
    glVertex3f(0.03, -0.02 + altura_conexao, -1.0)
    glVertex3f(0.03, -0.08 + altura_conexao, -1.0)  # Parte inferior do quadrado
    glVertex3f(-0.03, -0.08 + altura_conexao, -1.0)
    glEnd()
...
def main():
    ...
    while True:
        ...
        desenhar_corpo_central_unico(2, 2, 0.1, 50)
        desenhar_cauda()
