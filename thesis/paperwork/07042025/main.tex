...
\begin{itemize}
    ...
    T(n) = O(n^k)
    \]
    para alguma constante \( k \). Essa classe representa problemas considerados tratáveis na prática, como o MergeSort (\( O(n \log n) \)) \cite{knuth1998}.
    ...
    T_V(|x| + |y|) = O((|x| + |y|)^k)
    \]
    Exemplo: Satisfatibilidade Booleana (SAT) \cite{garey1979}.
    ...
    \]
    onde \( B \leq_P A \) indica uma redução polinomial. Exemplo: Problema da Mochila (Knapsack) \cite{garey1979}.
    ...
    \end{cases}
    \]
    O Teorema de Turing demonstra que HALT é indecidível, não existindo algoritmo que resolva o problema para todas as entradas \cite{sipser2012}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Popularidade}: Métricas como o TIOBE Index \cite{tiobe}, GitHub Octoverse \cite{octoverse} e Stack Overflow Developer Survey \cite{stackoverflow}.
    ...
    \item \textbf{Aplicabilidade acadêmica e industrial}: Referências como Sebesta \cite{sebesta2016} evidenciam o uso prático e teórico das linguagens.
\end{itemize}
...
\subsection{Conexão entre Algoritmos e Linguagens}
...
\begin{itemize}
    \item \textbf{Processador}: Intel Core i7-11800H, com 8 núcleos físicos e suporte a múltiplos threads, o que possibilita executar algoritmos em diferentes linguagens com capacidade computacional adequada.
    \item \textbf{Memória RAM}: 16 GB DDR4, garantindo que a execução dos experimentos ocorra sem restrições de memória para os tamanhos de entrada definidos.
    \item \textbf{Sistema Operacional}: Ubuntu 22.04 LTS, escolhido por sua estabilidade e suporte a múltiplas linguagens de programação, além de ferramentas de monitoramento de desempenho.
\end{itemize}
